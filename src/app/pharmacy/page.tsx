...
export default function PharmacyPage() {
    ...
    const fetchData = () => {
        fetch('/api/pharmacy')
            .then(r => {
                if (r.status === 403) { setError(t('pharmacy.error.access_denied')); setLoading(false); return []; }
                return r.json();
            })
            .then(d => { if (Array.isArray(d)) setDispensations(d); setLoading(false); })
            .catch(() => { setError(t('pharmacy.error.load_failed')); setLoading(false); });
    };
        ...
        return (
            <div className="space-y-6 animate-fade-in">
                ...
                <div className="card border-red-500/30 text-center py-12">
                    <p className="text-red-400 text-lg mb-2">🔒 {t('pharmacy.restricted')}</p>
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                <div className="card">
                    <p className="text-sm text-surface-400">{t('pharmacy.stats.dispensations')}</p>
                    <p className="text-2xl font-bold text-white mt-1">{dispensations.length}</p>
                </div>
                <div className="card">
                    <p className="text-sm text-surface-400">{t('pharmacy.stats.tablets_out')}</p>
                    <p className="text-2xl font-bold text-white mt-1">{totalDispensed}</p>
                </div>
                <div className="card">
                    <p className="text-sm text-surface-400">{t('pharmacy.stats.tablets_returned')}</p>
                    <p className="text-2xl font-bold text-white mt-1">{totalReturned}</p>
                </div>
                <div className="card">
                    <p className="text-sm text-surface-400">{t('pharmacy.stats.lots')}</p>
                    <p className="text-2xl font-bold text-white mt-1">{lotNumbers.length}</p>
                </div>
                <div className="card">
                    <p className="text-sm text-surface-400">{t('pharmacy.stats.pending')}</p>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-surface-700/50 bg-surface-800/50">
                                    <th className="text-left text-xs font-medium text-surface-400 uppercase tracking-wider px-6 py-3">{t('pharmacy.table.study_id')}</th>
                                    <th className="text-left text-xs font-medium text-surface-400 uppercase tracking-wider px-6 py-3">{t('pharmacy.table.treatment')}</th>
                                    <th className="text-left text-xs font-medium text-surface-400 uppercase tracking-wider px-6 py-3">{t('pharmacy.table.visit')}</th>
                                    <th className="text-left text-xs font-medium text-surface-400 uppercase tracking-wider px-6 py-3">{t('pharmacy.table.lot')}</th>
                                    <th className="text-left text-xs font-medium text-surface-400 uppercase tracking-wider px-6 py-3">{t('pharmacy.table.dispensed')}</th>
                                    <th className="text-left text-xs font-medium text-surface-400 uppercase tracking-wider px-6 py-3">{t('pharmacy.table.returned')}</th>
                                    <th className="text-left text-xs font-medium text-surface-400 uppercase tracking-wider px-6 py-3">{t('pharmacy.table.adherence')}</th>
                                    <th className="text-left text-xs font-medium text-surface-400 uppercase tracking-wider px-6 py-3">{t('pharmacy.table.date')}</th>
                                    <th className="text-right text-xs font-medium text-surface-400 uppercase tracking-wider px-6 py-3">{t('pharmacy.table.actions')}</th>
                                </tr>
                            </thead>
                            <tbody>
                                {dispensations.map(d => (
                                    <tr key={d.id} className="table-row">
                                        ...
                                        <td className="px-6 py-4 text-sm">
                                            {d.tabletsReturned != null ? (
                                                ...
                                                <span className="text-amber-400/60 text-xs">{t('pharmacy.status.pending')}</span>
                                            )}
                                        </td>
                                        <td className="px-6 py-4 text-sm">
                                ...
                                {dispensations.length === 0 && (
                                    <tr><td colSpan={9} className="text-center text-surface-500 py-8">{t('pharmacy.no_records')}</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
            ...
            {showDispense && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowDispense(false)}>
                    <div className="bg-surface-800 border border-surface-700/50 rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-5 animate-fade-in" onClick={e => e.stopPropagation()}>
                        ...
                        <div>
                            <label className="label">{t('pharmacy.field.participant')}</label>
                        ...
                        <div>
                            <label className="label">{t('pharmacy.field.visit')}</label>
                        ...
                        <div>
                            <label className="label">{t('pharmacy.field.lot')}</label>
                        ...
                        <div>
                            <label className="label">{t('pharmacy.field.dispensed')}</label>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setReturnDisp(null)}>
                    <div className="bg-surface-800 border border-surface-700/50 rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-5 animate-fade-in" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="bg-surface-700/30 rounded-lg p-3 space-y-1">
                            ...
                            <p className="text-xs text-surface-400">{t('pharmacy.field.dispensed')}: <span className="text-white font-medium">{returnDisp.tabletsDispensed} tablets</span></p>
                        </div>

                        <div>
                            <label className="label">{t('pharmacy.field.returned')}</label>
                            ...
                            {returnDisp.tabletsDispensed > 0 && (
                                <div className="mt-2 text-sm">
                                    <span className="text-surface-400">{t('pharmacy.table.adherence')}: </span>
                        ...
                        <div>
                            <label className="label">{t('pharmacy.field.notes')} <span className="text-surface-500 font-normal">({t('common.optional')})</span></label>
                            <textarea className="input min-h-[60px]" placeholder={t('pharmacy.notes_placeholder')}
                        ...
                        <div className="flex gap-3 justify-end pt-2">
                            ...
                            <button onClick={handleReturn} disabled={returning} className="btn-primary">
                                {returning ? t('common.saving') : t('pharmacy.btn.save_return')}
                            </button>
                        </div>
                    </div>
                </div>
